...
while time.time() < start+tlim:
        ...
        if redpanic:
			...
			if dc:
				...
				pservo.ChangeDutyCycle(1.6/21.6*100)
                        changed = False
	...
	i = 0
	while i < numrot:
		if ((time.time()-offsettime) % cycle)<rottime: #forward
			servo = True
			if redpanic and phase[0] == 1:
				pservo.ChangeDutyCycle(1.6/21.6*100)
				phase[0] = 0
				phase[1] = 1
		elif ((time.time()-offsettime) % cycle)<(rottime+pausetime): #stop
			pservo.ChangeDutyCycle(0)
			servo = False
